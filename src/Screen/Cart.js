...
export const Cart = () => {
  ...
  const NavigateToBackgroundimgPage = useNavigate();

  const [GetAllCustomerOrderList, SetGetAllCustomerOrderList] = useState();
  // sweet alert
  ...
  const GetRestaurantIDFromLocalStorages = () => {
    ...
    let RestID;
    const GetUserIDFromLocalStorage = JSON.parse(
      localStorage.getItem("restaurantsData")
    ).restID;
    console.log("GetUserIDFromLocalStorage", GetUserIDFromLocalStorage);
  ...
  const navigate = () => {
    ...
    NavigateToBackgroundimgPage("/backgroundimg");
  };
  ...
  const RemoveItemsFromCart = () => {
    axios
      .post("https://localhost:7089/api/Payment/api/Payment/" + UserID)
      .then((res) => {})
      .catch((e) => {
        alert("Wrong With RemoveItemFromCart API ");
      });
  };
  ...
  const GetAllCustomerOrder = () => {
    debugger;
    console.log("userid", userid);
    axios
      .get(
        "https://localhost:7089/api/CustomerOrder/GetAllCustomerOrdersByUserID/" +
          userid
      )
      .then((res) => {
        SetGetAllCustomerOrderList(res.data);
      })
      .catch((e) => {
        alert("Wrong With GetAllCustomerOrder API ");
      });
  };

  const SetQuantityForCustomerOrderProductOnCartDecrease = (item) => {
    debugger;
    ...
    axios
      .patch(
        "https://localhost:7089/api/CustomerOrderController/SetCustomerOrderQuantityOnCart",
        obj
      )
      .then((res) => {
        GetAllCustomerOrder();
      ...
      .catch((e) => {
        alert("Wrong With SetQuantityForCustomerOrderProductOnCart API");
      });
  };

  const SetQuantityForCustomerOrderProductOnCartIncrease = (item) => {
    debugger;
  ...
  const CartCountItems = () => {
    debugger;
    let CartLength = 0;
    GetAllCustomerOrderList?.forEach((order) => {
      CartLength += order.customerOrderProductName.length;
    });
    console.log("CartLength", CartLength);
    return CartLength;
  };

  const RemoveItemFromCart = (customerOrderID) => {
    debugger;

    axios
      .delete(
        "https://localhost:7089/api/CustomerOrder/RemoveItemFromCart/" +
          customerOrderID
      )
      .then((res) => {
        GetAllCustomerOrder();
  ...
  const ChangeHandleForRatings = (e) => {
    debugger;
    ...
    const obj = {
      restID: StoreRestID,
      feedBack: FeedBackint,
    };

    console.log("obj", obj);
    debugger;
    axios
      .patch("https://localhost:7089/api/restaurant/GetFeedBackFromUser", obj)
      .then((res) => {
        pay();
        navigate();
      ...
      .catch((e) => {
        alert("Wrong With ChangeHandleForRatings API ");
      });
  };
  ...
  return (
    <div>
        ...
        <div className="container h-100">
          <div className="row d-flex justify-content-center align-items-center h-100">
            <div className="col">
              ...
              {GetAllCustomerOrderList?.map((item) => (
                <div>
                  <div className="card-body p-4">
                    <div className="row align-items-center">
                   
                      <div className="col-md-2" key={item.customerOrderID} > 
                        <img
                          src={`data:image/jpg;base64, ${item.customerOrderFoodImage} `}
                          height="100"
                          width="100"
                        />
                      </div>
                        ...
                        <div>
                          ...
                          <div className="d-flex align-items-center justify-content-center">
                            <button
                              ...
                              onClick={() =>
                                SetQuantityForCustomerOrderProductOnCartDecrease(
                                  item
                                )
                              }
                            >
                            ...
                            <p className="lead fw-normal mb-0 mx-3">
                              {item.customerOrderQuantity}
                            </p>
                            <button
                              className="btn btn-secondary"
                              onClick={() =>
                                SetQuantityForCustomerOrderProductOnCartIncrease(
                                  item
                                )
                              }
                            >+</button>
                      ...
                      <button
                        ...
                        onClick={() => RemoveItemFromCart(item.customerOrderID)}
                      >
                        Remove
                      </button>
                    </div>
              ...
              {GetAllCustomerOrderList?.forEach((item) => {
                grandTotal +=
                  item.customerOrderPrice * item.customerOrderQuantity;
              })}
      ...
      <div className="modal" id="newPaymentModal" role="dialog">
        <div className="container">
          <div className="feedback">
            <div className="rating" onChange={ChangeHandleForRatings}>
              <input data-dismiss="modal" type="radio" value={5} name="rating" id="rating-5" />
              <label htmlFor="rating-5"></label>
              <input data-dismiss="modal" type="radio" value={4} name="rating" id="rating-4" />
              <label htmlFor="rating-4"></label>
              <input data-dismiss="modal" type="radio" value={3} name="rating" id="rating-3" />
              <label htmlFor="rating-3"></label>
              <input data-dismiss="modal" type="radio" value={2} name="rating" id="rating-2" />
              <label htmlFor="rating-2"></label>
              <input data-dismiss="modal" type="radio" value={1} name="rating" id="rating-1" />
